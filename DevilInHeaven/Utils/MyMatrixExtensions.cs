...
namespace Black_Magic
{
    public static class MyMatrixExtensions
    {
        ...
        //Returns the byte matrix of a given dimension
        internal static byte[,] GetDimensionMatrix(this byte[,,] given, int dim)
        {
            byte[,] toReturn = new byte[given.GetLength(0), given.GetLength(1)];
            for (int x = 0; x < given.GetLength(0); x++)
            {
                for (int y = 0; y < given.GetLength(1); y++)
                {
                    toReturn[x, y] = given[x,y,dim];
                }
            }
            return toReturn;
        }
